...
> Подход: “умный клиент”, который снимает рутину (файлы, патчи, прогресс, скачивание), но остаётся прозрачным и поддерживаемым.
...
### 4.3 WS reconnect
- Автопереподключение с backoff (без бесконечных попыток).
...
По принятому правилу: если матчей > 1 — это ошибка (без неявного выбора “первого попавшегося”).
